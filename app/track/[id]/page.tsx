...
export default function BookingTrackerPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      {/* Automated CRM & SMS / WhatsApp Notification Stream (UC-07, US-03, FR-05) */}
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-3">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
          <div className="flex items-center gap-2">
            <MessageSquare className="h-4 w-4 text-emerald-600" />
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800">
              Automated CRM & SMS/WhatsApp Notification Feed
            </h3>
          </div>
          <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 flex items-center gap-1">
            <Send className="h-2.5 w-2.5" /> Webhook Alerts Active
          </span>
        </div>

        <div className="space-y-2">
          {/* 1. Deposit Confirmation Alert */}
          ...
          {/* 3. En Route ETA Alert */}
          {['en_route', 'in_progress', 'completed', 'billed'].includes(booking.status) && technician && (
            <div className="rounded-lg border border-purple-200 bg-purple-50/40 p-3 text-xs space-y-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 font-bold text-purple-900">
                  <Navigation className="h-3.5 w-3.5 text-purple-600" />
                  <span>Real-Time Travel & ETA Alert</span>
                </div>
                <span className="text-[10px] text-purple-700 bg-purple-100 font-bold px-1.5 py-0.2 rounded font-mono border border-purple-200">
                  Live Dispatched
                </span>
              </div>
              <p className="text-slate-800 text-[11px] font-mono leading-relaxed bg-white p-2 rounded border border-purple-200">
                "En Route: {technician.full_name} has departed and is traveling to your location. Estimated arrival in ~20 minutes."
              </p>
            </div>
          )}

          {/* 4. Completion & Tax Invoice Alert */}
          {['completed', 'billed'].includes(booking.status) && (
            <div className="rounded-lg border border-emerald-200 bg-emerald-50/40 p-3 text-xs space-y-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 font-bold text-emerald-900">
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" />
                  <span>Digital Invoice & Sign-Off Alert</span>
                </div>
                <span className="text-[10px] text-emerald-700 bg-emerald-100 font-bold px-1.5 py-0.2 rounded font-mono">
                  Settled
                </span>
              </div>
              <p className="text-slate-800 text-[11px] font-mono leading-relaxed bg-white p-2 rounded border border-emerald-200">
                "Service Complete: Digital sign-off verified. Your official tax invoice INV-{booking.id.slice(-6).toUpperCase()} is ready for download."
              </p>
            </div>
          )}
        </div>
